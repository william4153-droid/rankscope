...
async function listOverview(adminClient) {
  ...
  const [profilesResult, usageResult, searchesResult] = await Promise.all([
    ...
    adminClient
      .from("search_history")
      .select(
        "id,user_id,platform,research_goal,keyword,country,language,competition_score,competition_label,search_intent,result_count,searched_at,profiles(email,full_name)"
      )
